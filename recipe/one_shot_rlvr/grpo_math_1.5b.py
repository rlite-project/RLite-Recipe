...
class GrpoConfig:
    ...
    # wandb config
    wandb_project: str = "one-shot-rlvr-reproduce"
    wandb_name: str = "qwen2.5-math-1.5b-zero-entropy"
    ...
    max_steps: int = 2000
    eval_interval: int = 5
    entropy_bonus_weight: float = -1e-3
...
class GrpoPolicyActor(RliteTrainModule, Qwen2ForCausalLM):
    ...
    def grpo_train_step(
        ...
        entropy = self.get_entropy(logp, action_mask)
        loss += GrpoConfig.entropy_bonus_weight * entropy
        loss.backward()

        return self.collect_metrics(loss, chosen_logp, action_mask, entropy)
...
class SimpleGrpoExp:
    def run(self):
        rlite.init()
        self.init_wandb()
        for step in range(GrpoConfig.max_steps):
            self.global_index = step
            if self.global_index % GrpoConfig.eval_interval == 0:
                self.evaluate()
            self.train_step(*self.train_data)
    ...
    def train_data(self):
        prompt = (
            "The pressure \\( P \\) exerted by wind on a sail varies jointly "
            "as the area \\( A \\) of the sail and the cube of the wind's "
            "velocity \\( V \\). When the velocity is \\( 8 \\) miles per "
            "hour, the pressure on a sail of \\( 2 \\) square feet is "
            "\\( 4 \\) pounds. Find the wind velocity when the pressure on "
            "\\( 4 \\) square feet of sail is \\( 32 \\) pounds. Let's think"
            " step by step and output the final answer after \"####\"."
        )
        return (
            [prompt] * GrpoConfig.num_prompts_per_batch,
            ["12.8"] * GrpoConfig.num_prompts_per_batch
        )
